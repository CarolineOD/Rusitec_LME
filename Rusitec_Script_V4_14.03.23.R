...
output <- data.frame(as.matrix(nrow=, ncol=))
colnames(output) <- c()

for (var in c("Gas_vol_L", "CH4_pc", "CH4_vol_L")){
  formula <- as.formula(paste0(var, "~ RunDay + Treatment"))
  model <- lme(formula,
               random=~1|Vessel,
               data=df,
               method = 'REML')

  output[nrow(output)+1] <- c(var, summary(model)[3,3])  
}
...
mod1.1 <- lme(CH4_vol_L~RunDay+Treatment,random=~1|Vessel,
            data=df,
            method='REML')
#mod2 if you want to see if the treatment effect changes over time then fit the interaction model (Andrew)
mod2.1 <- lme(CH4_vol_L~RunDay*Treatment,random=~1|Vessel,
            data=df,
            method='REML')
...
#emmeans and lsmeans are just two different ways to compare treatments and determine whether contrast is significant via p value
emmeans(mod1.1, pairwise~Treatment) ### post hoc est marginal means
lsmeans(mod1.1, pairwise~Treatment) ### least square means (used in SAS script)

emmeans(mod2.1, pairwise~Treatment) ### post hoc est marginal means
lsmeans(mod2.1, pairwise~Treatment) ### least square means (used in SAS script)
...
df$res2 = residuals(mod2)
df$pred2 = predict(mod2, df)

hist(df$res2) # should be symmetric
plot(df$pred2, df$res) # should be a random scatter about 0




df$res1.1 = residuals(mod1.1)
df$pred1.1 = predict(mod1.1, df)

hist(df$res1.1) # should be symmetric
plot(df$pred1.1, df$res) # should be a random scatter about 0

df$res2.1 = residuals(mod2.1)
df$pred2.1 = predict(mod2.1, df)

hist(df$res2.1) # should be symmetric
plot(df$pred2.1, df$res) # should be a random scatter about 0
...
#######################################################################################################
#NH3 analysis
#added vessel & day to be able to run through LME. Overkill for these data sets maybe, but will compare to ANOVA and see

NH3 = read.table('Ammonia.csv', header=T, sep=',')
Identification_key = read.table('NH3_VFA_Vessel_numbers.csv', header=T, sep=',')

NH3new = merge(NH3, Identification_key, by = 'Sample_ID')
head(NH3new)

#######################################################################################################
#VFA analysis
VFA = read.table('VFA_tidy_results.csv', header = T, sep = ",")
#can't merge VFA and NH3 as they don't have the same number of data points for each variable 
head(VFA)
