...
ggplot(filtered, aes(Treatment, Vessel_pH, fill = Treatment)) + # define variables
  geom_boxplot(size=1) 
...
filtered$Treatment <- factor(filtered$Treatment, levels = c("Control","0.5X UHP+KI", "0.5X UHP", 
                                                             "0.25X UHP+KI", "0.25X UHP ", "0.5X MgO2"))

# now when we plot it again, our treatments should be in the order we want.
ggplot(filtered, aes(Treatment, Vessel_pH, fill = Treatment)) + 
  geom_boxplot() 
...
plot1 = ggplot(filtered, aes(Treatment, Vessel_pH, fill = factor(Treatment))) + # define variables
